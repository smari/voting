<template>
  <b-form>
    ...
    <b-row>
      <b-col>
        <b-form-group label="Divider for allocating constituency seats" description="Which divider rule should be used to allocate constituency seats to lists within each constituency?">
          <b-form-select v-model="rules.election_rules.primary_divider" :options="rules.capabilities.divider_rules" class="mb-3"/>
        </b-form-group>
        <b-form-group label="Divider for apportioning adjustment seats" description="Which divider rule should be used to apportion adjustment seats among parties?">
          <b-form-select v-model="rules.election_rules.adj_determine_divider" :options="rules.capabilities.divider_rules" class="mb-3"/>
        </b-form-group>
        <b-form-group label="Divider for allocating adjustment seats" description="Which divider rule should be used to allocate adjustment seats to individual lists?">
